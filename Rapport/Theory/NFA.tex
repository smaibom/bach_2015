...
\label{RA_TO_NFA}
Each expression can be converted to an NFA, and vice versa. Table~\ref{tab:NFA_TAB} shows how each RE can be converted into a NFA. The states in $Q$ is represented as circles. The initial state $q^s$ is shown by adding a small arrow pointing to it, and the accepting state $q^a$ is shown as a double circle. Transitions in $\Delta$ is represented as  $(q,a,q')\in \Delta$ or $(q,\epsilon ,q')\in\Delta$, we write these transitions as $q \xrightarrow{a} q'$ or $q \xrightarrow{\epsilon} q'$. For subexpressions we use boxes and split the transition arrow, marking it with an $E$, to denote the NFA resulted from converting the expression. 
%\\
...
\begin{myex}
In example \ref{reinter}, the expression $c(a+b)^*$ is used. The NFA buildup is done like the language interpetation in the example. We first construct the $E_1E_2$ expression, where $E_1 = c$ and $E_2 = (a+b)^*$.
