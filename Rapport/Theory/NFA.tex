...
\label{RA_TO_NFA}
Each expression can be converted to an NFA, and vice versa. Table~\ref{tab:NFA_TAB} shows how each RE can be converted into an NFA. The states in $Q$ are represented as circles. The initial state $q^s$ is shown by adding a small arrow pointing to it, and the accepting state $q^a$ is shown as a double circle. Transitions in $\Delta$ are represented as  $(q,a,q')\in \Delta$ or $(q,\epsilon ,q')\in\Delta$, we write these transitions as $q \xrightarrow{a} q'$ or $q \xrightarrow{\epsilon} q'$. For subexpressions we use boxes and split the transition arrow, marking it with an $E$, to denote the NFA resulted from converting the expression. 
%\\
...
\begin{myex}
In example \ref{reinter}, the expression $c(a+b)^*$ was used. The NFA buildup is done like the language interpretation in the example. We first construct the $E_1E_2$ expression, where $E_1 = c$ and $E_2 = (a+b)^*$.
...
\end{tikzpicture}\\
\end{center}
The last part is constructing $E_3$, we skip ahead and construct the literals {\tt a} and {\tt b} as well. 
...
\begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=1.5 cm, scale = 0.75, transform shape,initial text={}]
  ...
  \node [state,above right of=0,xshift=2.5cm,yshift=2cm] (3) {};
  \node [state,below right of=2,xshift=1.25cm] (5) {};
  \node [state,above right of=2,xshift=1.25cm] (6) {};
  \node [state,right of=5,xshift=1.25cm] (7) {};
  \node [state,right of=6,xshift=1.25cm] (8) {};
  ...
  \path[->] (0) edge node {$\epsilon$} (1);
  \path[->] (3) edge [bend left] node {$\epsilon$} (0);
  \path[->] (0) edge [bend left] node {$\epsilon$} (2);
  \path[->] (99) edge node {{\tt c}} (0);
\end{tikzpicture}\\
\end{center}
