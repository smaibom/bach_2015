...
bool simulate(World* NFA,std::string string){
	std::list<Node*> statelist;
	int i = 0;
	Node* startnode = NFA->start;
	Node* curnode;
	...
	while(! nodes.empty()){
		curnode = nodes.front();
		nodes.pop();
		if(curnode->nexts.empty()){
			for (std::list<Node*>::iterator it = curnode->epsilons.begin(); it != curnode->epsilons.end(); it++){
				nodes.push(*it);
			}
		}
		else{
			map<char,Node*>::const_iterator it = curnode->nexts.find(string[i]);
			
			if(it!=curnode->nexts.end()){
				statelist.push_front(it->second);
			}
		}
	}
	if(statelist.empty()){
